...
**新增**
- 📚 **服务器地址簿**:多服务共用一个壳(Pi Web / CCR 控制台 / 任意自托管服务)
  - π 菜单顶部 = 快切列表,当前服务 ✓ 标记,点一下即切换并重载
  - 「服务器地址簿…」管理页: 点按连接 / 长按编辑删除 / 底部「＋ 添加新地址」
  - 地址复用 v2.0 校验(IPv4 / [IPv6] / 域名 / Tailscale MagicDNS,可省略 http://),
    支持带查询参数的 URL(如 CCR 的 `?ccr_web_token=***  - 升级自动迁移: 旧版已有地址存为首条「Pi Web」,无感升级
  - 错误页「修改地址」改址后,地址簿对应条目跟随更新,不会失配
- 菜单标题显示当前服务名(`Pi Web · CCR 控制台`)
- 日志新增 Book 通道: 迁移/添加/编辑/删除/切换全记录

**CCR 控制台集成说明**:CCR Web UI 的 token 随 URL 带入即可,UI 自行转换鉴权头,
APP 侧零专属代码;受 CCR Host 白名单限制,入口需经服务端 L7 反代链(如 Tailscale 转发)。
